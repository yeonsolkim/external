...
&emsp;&emsp;An *event* is a collection of outcomes, so mathematically an event is a subset of the sample space. For example, if one die is rolled, the event that an even number appears is
