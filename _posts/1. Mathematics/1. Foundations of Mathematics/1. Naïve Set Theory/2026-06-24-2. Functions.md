...
The *domain* of $R$ is the set $\lbrace x : \exists y \in B\  ((x,y)\in R)\rbrace,$ and the *image* of $R$ is the set $\lbrace y : \exists x \in A \ ((x,y)\in R)\rbrace.$
...
The domain $A$ of $R$ is the *domain* of $f,$ and the image of $R$ is the *image* of $f;$ the set $B$ is the *codomain* of $f.$ If $a\in A,$ the *value* of $f$ at $a$ is the unique element of $B$ to which $R$ assigns $a.$
...
Let $b\in B.$ Then there exists $a$ such that $f(a)=b.$ Since $g(b)=g(f(a))=a,$ $g$ is the inverse of $f.$ Since $f(a)=b$ and $f(h(b))=b,$ by the definition of injectivity, $a=h(b).$ Thus $h$ is also the inverse of $f.$ <span class="qed">$\square$</span>
...
We can also define set-level operations of the image operation and the preimage operation from the above.
...
*Proof.* (1) If $b\in f[A_0],$ there exists $a\in A_0$ such that $f(a)=b.$ Since $A_0\subseteq A_1,$ it follows that $a\in A_1.$ Thus $b=f(a)\in f[A_1].$ Therefore $f[A_0]\subseteq f[A_1].$
...
*Proof.* It is not quite different from Theorem 1.2.9 to prove this.<span class="qed">$\square$</span>
...
We now suppose that $f$ is surjective. Thus there exists $a\in A$ such that $f(a)=b$ if $b\in B_0.$ By Definition 1.2.8, $a\in f^{-1}[B_0],$ thus $f(a)\in f[f^{-1}[B_0]].$ Therefore $B_0 \subseteq  f[f^{-1}[B_0]].$ Given $(\ast \ast ),$ it follows that $f[f^{-1}[B_0]]= B_0.$ <span class="qed">$\square$</span>
