...
last_modified_at: 2026-09-13 15:51:04 +0900
---
...
*Proof.* (1) Let $x\cdot w=1.$ Since $(x\cdot y)\cdot w = (x\cdot z)\cdot w,$ $(x\cdot w)\cdot y = (x\cdot w)\cdot z.$ Hence $y=z.$
...
(4) Since $x+((-1)\cdot x)=(1+(-1))\cdot x = 0,$ by (3) in Proposition 1.1.3, $(-1)\cdot x = -x.$<span class="qed">$\square$</span>
...
An $\mathcal L_{\mathrm{field}}$-structure ${\mathcal F}=(F, 0, 1, +, -, \cdot)$ is called a *field* if and only if ${\mathcal F}\vDash T_{\mathrm{field}},$ and $x\in F$ is called a *scalar*.
...
(4) Since the order is comparable if $0\ne x$ then $0<x$ or $x<0.$ If $0<x$ then $0\cdot x<x\cdot x,$ so that $0<x^2.$ If $x<0$ then $0<x^2.$ The only case left is that $x=0.$ If $x=0$ then $x^2=0\cdot x=0.$ Therefore $0\le x^2.$ <span class="qed">$\square$</span>

**Theorem 1.1.8.** The real number system $\mathcal R=(\mathbb R,0,1,+,-,\cdot,<)$ has the least-upper-bound property. Also, $\mathcal R \vDash T_{\mathrm{of}}.$
...
**Lemma 1.1.10.** Let $A\subseteq \mathbb R$ be bounded above, and let $\alpha=\sup A$. Then, for all $\varepsilon>0$, there exists $x\in A$ such that

$$ \alpha-\varepsilon<x<\alpha.$$

*Subproof.* If there are no elements in the interval $(\alpha-\varepsilon, \alpha],$ then $\alpha-\varepsilon$ is an upper bound for $A$, which is a contradiction to the premise that $\alpha$ is the supremum.<span class="qed">$\blacksquare$</span>
...
*Proof.* Let $x,y\in \mathbb R^{+}$ be given. We let $a=(x\cdot y)^{1/n}, b=x^{1/n},$ and $c=y^{1/n}.$ Then we have $a^n=x\cdot y, b^n=x,$ and $c^n=y.$ Hence $a^n=b^n \cdot c^n=(b\cdot c)^n,$ so that
