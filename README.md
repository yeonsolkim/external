...
- The penultimate folder in a post's `category_path` represents its textbook;
  the final folder represents a section within that textbook. Only the section
  folder shows its ordering prefix in category entries and breadcrumbs; every
  other level (top-level, intermediate folders, and the textbook itself) hides
  it. Sections are detected as the one level without a generated index page.
...
- `assets/js/main.js` is a Jekyll-processed JavaScript file. The leading front
  matter markers are intentional because it embeds the compact reference index
  generated by `_plugins/reference_label_targets.rb`.
- The build scans posts for labels such as `Definition 1.3.9` and
  `Exercise 1.3.9`, and the browser links fully qualified references to the
  matching label in the same `data-reference-scope`. A bare number such as
  `1.3.9` is linked only when it identifies exactly one target in that scope.
- If theorem/definition links disappear, check the `textbook_category` filter,
  `_plugins/reference_label_targets.rb`, `_layouts/post.html`, and the embedded
  `referenceTargetsByScope` in `assets/js/main.js` still agree.
